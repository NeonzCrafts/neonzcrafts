...
function initCheckoutHandlers(){
  ...
  $("place-order")?.addEventListener("click",async()=>{
    ...
    const addr=safeParse(localStorage.getItem(STORAGE_KEYS.address),null); if(!addr) return alert("Save address first");
    try{
      await emailjs.send(EMAILJS_SERVICE,EMAILJS_TEMPLATE,{
        order_items: cart.map(it=>{
          const p=PRODUCTS.find(pr=>pr.id===it.id)||{}; return `${p.title} × ${it.qty} = ${formatCurrency((p.price||0)*it.qty)}`;
        }).join("\n"),
        order_total: formatCurrency(cart.reduce((s,it)=>{const p=PRODUCTS.find(pr=>pr.id===it.id)||{price:0};return s+p.price*it.qty;},0)),
        customer_name: addr.name,
        customer_phone: addr.phone,
        customer_address: `${addr.street}, ${addr.city} - ${addr.pincode}`
      });
      clearCart(); window.location.href="order-success.html";
    }catch(err){console.error("EmailJS error",err);alert("Failed to send order confirmation.");}
  });
}
...
document.addEventListener("DOMContentLoaded",()=>{
  ...
  if($("address-form")||$("place-order")){ renderCheckoutPage(); initCheckoutHandlers(); }
  initBrandLinks();
  window.addEventListener("storage",()=>{updateCartBadge(loadCart()); if($("cart-items")) renderCartPage(); if($("subtotal-amt")) renderCheckoutPage();});
  window.addEventListener("neon:cart:updated",()=>{updateCartBadge(loadCart()); if($("cart-items")) renderCartPage(); if($("subtotal-amt")) renderCheckoutPage();});
});
